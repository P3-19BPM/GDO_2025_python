...
# "E:\QGis\UrbanoRural\Rural_19BPM.json"
geojson_path = r"E:\QGis\UrbanoRural\Urb_Rural_15RPM_CGA\Urb_Rural_15RPM_CGA_GeoJson.json"
# "E:\QGis\UrbanoRural\BD_2020-2024_memorando_018-15RPM.xlsx"
output_excel_path = r"E:\Pertubacao.xlsx"

# Dicionário com os arquivos e abas correspondentes
excel_sheets = {
    "BD_tratado": {
        "file_path": output_excel_path,
        "sheet_name": "URB_RURAL"
...
def process_data(input_path, sheet_name, geojson_path):
    try:
        ...
        print(
            f"Valores únicos em 'SITUACAO': {polygons_gdf['SITUACAO'].unique()}")
        ...
        if 'SITUACAO' in result_gdf.columns:
            result_gdf['URB_RURAL'] = result_gdf['SITUACAO'].str.upper().apply(
                lambda x: 'Urbana' if 'URBAN' in str(x) else 'RURAL')
        else:
            available_cols = result_gdf.columns.tolist()
            raise ValueError(
                f"Coluna 'SITUACAO' não encontrada. Colunas disponíveis: {available_cols}")
        ...
        result = result_gdf[original_cols + ['URB_RURAL']]
